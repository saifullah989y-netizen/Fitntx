...
async function loadPartials() {
  ...
  if (headerEl) {
    ...
    headerEl.querySelectorAll('.nav-links a').forEach(a => {
      ...
      const isShop =
        href === '/shop' &&
        (currentFile === 'shop.html' || currentFile === 'shop');
      if (href === currentFile || (currentFile === '' && href === 'index.html') || isStartHere || isShop) {
...
// ── Stories carousel (auto + arrow controls) — each .stories-shell is independent ──
function initStoriesCarousel() {
  document.querySelectorAll('.stories-shell').forEach((shell) => {
...
const results = {
  ...
  athlete_strategy:       { h:'Your Path: Career Strategy',          p:'Your Senior Representative will run a structured Initial Career Strategy Consultation — covering your current stage, goals, timeline, constraints, and a clear roadmap for the next 6–18 months.', cta:'Book My Strategy Call', page:'/start', paths:[{icon:'🗺️',t:'Career Roadmap Session',d:'Goals + next steps'},{icon:'🏃',t:'Athlete Membership',d:'Ongoing Rep support'},{icon:'📈',t:'Placement Pipeline',d:'Active opportunity search'}] },
  athlete_placement:      { h:'Your Path: Opportunity Placement',    p:'Your Representative will actively work your placement — matching you to clubs, trials, contracts, and roles that fit your level, sport, and ambitions globally.', cta:'Start My Placement', page:'/start', paths:[{icon:'🚀',t:'Active Placement',d:'Rep works your pipeline'},{icon:'🌍',t:'Global Network',d:'Opportunities worldwide'},{icon:'🏃',t:'Pro Membership',d:'Priority Rep support'}] },
  athlete_network:        { h:'Your Path: Network & Hub',            p:'Get connected to the FITNTX Hub — the spaces, events, and people that drive real relationships in sport. Your Rep will introduce you to the right people in your discipline and region.', cta:'Explore the Hub', page:'shop.html', paths:[{icon:'🌐',t:'Hub Spaces',d:'Sport-specific communities'},{icon:'📅',t:'Networking Events',d:'Real-world connection'},{icon:'🤝',t:'Rep Introductions',d:'Warm connections'}] },
  ...
  organization_talent:    { h:'Your Path: Talent Acquisition',       p:'A Senior Recruitment Representative will scope your talent needs, design your recruitment campaign, and manage the full pipeline — from sourcing to placement.', cta:'Book an Org Discovery Call', page:'/start', paths:[{icon:'🔍',t:'Talent Acquisition',d:'Full recruitment service'},{icon:'📋',t:'Scoping & Pricing',d:'Confirmed at discovery'},{icon:'🏢',t:'Org Package Option',d:'Bundle + save'}] },
  organization_partnerships:{ h:'Your Path: Partnerships & Sponsorships', p:'Your Representative will package your assets, identify the right partners, build the pitch, and close deals — structured around your commercial objectives and sport context.', cta:'Book an Org Discovery Call', page:'/start', paths:[{icon:'🤝',t:'Partnerships Service',d:'Strategy to close'},{icon:'💰',t:'Retainer + Success Fee',d:'Aligned incentives'},{icon:'🏢',t:'Org Package Option',d:'Bundle + save'}] },
  organization_events:    { h:'Your Path: Events Management',        p:'FITNTX plans and delivers events of any scale — from local clinics to international showcases. Your Rep will scope the brief, coordinate logistics, and deliver the outcome.', cta:'Book an Events Call', page:'/start', paths:[{icon:'📅',t:'Events Management',d:'End-to-end delivery'},{icon:'📋',t:'Get a Quote',d:'Scoped at discovery'},{icon:'🏢',t:'Org Package Option',d:'Events + more, bundled'}] },
  organization_growth:    { h:'Your Path: Strategic Growth',         p:'Start with an Organizational Growth Strategy Consultation. Your Rep will map your priorities, challenges, and constraints — and recommend whether a full annual package or specific service fits best.', cta:'Book an Org Strategy Call', page:'/start', paths:[{icon:'⚡',t:'Org Growth Consultation',d:'Your starting point'},{icon:'🏢',t:'Annual Packages',d:'Custom / yr'},{icon:'📋',t:'Custom Solutions',d:'Scoped to your org'}] }
};
...
function selectStep2(choice) {
  ...
  document.getElementById('flowResultInner').innerHTML = `
    ...
    <div class="result-actions">
      ...
      <a class="btn btn-out" href="/start">Book a Discovery Call</a>
    </div>
